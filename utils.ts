...
export const formatVoVideoId = (raw: string): string => {
  // Desired: vid017.01 (keep dot)
  const cleaned = (raw || '').trim();
  if (!cleaned) return 'vid###.##';
  // Keep the user's dot format, just prefix with `vid`
  return `vid${cleaned}`;
};
...
export const sortProductCodes = (codes: string[]): string[] => {
  const uniq = Array.from(
    new Set((codes || []).map(c => (c || '').trim().toUpperCase()).filter(Boolean))
  );

  const rank = (code: string): number => {
    if (code === 'ACN') return 0;
    if (code === 'FOU') return 2;
    if (code === 'CNC') return 3;
    return 1;
  };
  ...
  return uniq.sort((a, b) => {
    ...
    // Keep things predictable inside the same bucket
...
export const generateFilename = (type: FileTypeKey, data: FormState): string => {
  ...
  const _composer = composerName ? composerName.trim().replace(/\s+/g, '') : '';
  ...
  const _aiPrds = (aiProducts && aiProducts.length > 0)
    ? sortProductCodes(aiProducts).join('_')
    : _prd;
...
case 'screenshot': {
  ...
  return `${base} (${FULL_DATE})`;
    ...
    case 'ai': {
      ...
      // 2. Product Logic
      if (!aiNoVisibleProduct) {
        // If user selected products, include them. Otherwise fall back to PRD.
        parts.push(_aiPrds);
      }
    ...
    case 'vo': {
      ...
      const voVid = formatVoVideoId(_vidNum);
      const base = `VO_${_prd}_${voVid}_${_seq}_${identifier}_${_keys}`;
