        ...
        return (
          <>
            ...
            <div className="md:col-span-2 pt-1 pb-2">
              <Checkbox
                label="No visible product"
                checked={formData.aiNoVisibleProduct}
                onChange={(v) => {
                  setField('aiNoVisibleProduct', v);
                  if (v) {
                    setField('aiProducts', []);
                    setField('isProductVisible', false);
                  }
                }}
                desc="Omits product codes and hides product selection"
              />
            </div>

            {commonDate}
            
            {/* Products (multi) unless explicitly set to "no visible product" */}
            {!formData.aiNoVisibleProduct && (
              <div className="md:col-span-2">
                <MultiSelect
                  label="Products Visible"
                  options={PRD_OPTIONS}
                  values={formData.aiProducts}
                  onChange={(vals) => {
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        {showLegend && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-zinc-950 border border-white/10 rounded-2xl w-full max-w-5xl max-h-[85vh] flex flex-col shadow-2xl">
              ...
              {/* Sticky page switcher */}
              <div className="px-6 pt-5 pb-4 border-b border-white/5 bg-zinc-950/95 backdrop-blur-xl sticky top-0 z-10">
                <div className="flex items-center justify-between gap-4">
                  <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-500">Pages</div>
                  <div className="flex items-center gap-2">
                    <button
                ...
                {lexiconPage === 'lexicon' ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        <SectionLabel>Hook Theme</SectionLabel>
                        <div className="space-y-2">
                          {HOOK_THEME.map(item => (
                            <div key={item.code} className="text-xs text-zinc-300 border border-white/5 rounded-lg px-3 py-2 bg-white/5">
                              <div className="flex items-center justify-between">
                                <span className="font-mono font-bold text-brand">{item.code}</span>
                                <span className="text-zinc-400">{item.title}</span>
                              </div>
                              {item.desc && <div className="text-zinc-500 mt-1">{item.desc}</div>}
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div>
                        <SectionLabel>Intro Theme</SectionLabel>
                        <div className="space-y-2">
                          {INTRO_THEME.map(item => (
                            <div key={item.code} className="text-xs text-zinc-300 border border-white/5 rounded-lg px-3 py-2 bg-white/5">
                              <div className="flex items-center justify-between">
                                <span className="font-mono font-bold text-brand">{item.code}</span>
                                <span className="text-zinc-400">{item.title}</span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {CONCEPT_THEME_GUIDES.map(g => (
                      <div key={g.code} className="bg-white/5 border border-white/10 rounded-2xl p-6">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <div className="text-xs font-bold text-brand tracking-widest">{g.code}</div>
                            <div className="text-2xl font-bold text-white leading-tight">{g.title}</div>
                          </div>
                          <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-400">{g.formats}</div>
                        </div>
                        <div className="text-zinc-400 text-sm mt-3">{g.goal}</div>
                        <div className="text-zinc-500 text-xs mt-2 leading-relaxed">{g.summary}</div>

                        {g.whatIs.length > 0 && (
                          <div className="mt-5">
                            <div className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-2">What is</div>
                            <div className="space-y-2">
                              {g.whatIs.map(step => (
                                <div key={step.label} className="flex items-start justify-between gap-4 text-xs border border-white/5 rounded-xl px-3 py-2 bg-black/20">
                                  <div className="font-mono font-bold text-brand shrink-0">{step.label}</div>
                                  <div className="text-zinc-400 leading-relaxed">{step.desc}</div>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {g.note && (
                          <div className="mt-4 text-xs text-zinc-500 border border-white/5 rounded-xl px-3 py-2 bg-black/20">
                            {g.note}
                          </div>
                        )}
                      </div>
