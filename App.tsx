      ...
      case 'audio_assets':
        return (
          <>
            ...
            {platformInput}
            {formData.subType === 'music' 
              ? (<>
                  <TextInput label="Song Name" placeholder="SongName" value={formData.songName} onChange={(e) => setField('songName', e.target.value)} />
                  <TextInput label="Composer" placeholder="ComposerName" value={formData.composerName} onChange={(e) => setField('composerName', e.target.value)} />
                </>)
              : <TextInput label="Description" placeholder="WindChimes" value={formData.keywords} onChange={(e) => setField('keywords', e.target.value)} />
            }
          </>
        );
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-zinc-950 border border-white/10 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl">
              <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
                <h3 className="text-xl font-bold text-white">Legend / Data Dictionary</h3>
                <button onClick={() => setShowLegend(false)} className="text-zinc-400 hover:text-white">Close</button>
              </div>
              <div className="overflow-y-auto p-6 space-y-8">
                <div>
                  <SectionLabel>Products (PRD)</SectionLabel>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {PRD_OPTIONS.map(o => (
                      <div key={o.value} className="text-xs text-zinc-400 border border-white/5 rounded p-2 bg-white/5">
                        <span className="text-brand font-bold block">{o.value}</span>
                        {o.desc}
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  <SectionLabel>Scene Types</SectionLabel>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {SCENE_OPTIONS.map(o => (
                      <div key={o.value} className="text-xs text-zinc-400 border border-white/5 rounded p-2 bg-white/5">
                        <span className="text-brand font-bold block">{o.value}</span>
